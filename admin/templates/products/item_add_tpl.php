                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <div class="input-group-append">
                                                <div class="input-group-text">
                                                    <input type="checkbox" id="checkUrl<?=$k?>" class="alias_<?=$k?>" checked>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="font-weight-600">Giá Tour</label>
                                    <input type="text" class="form-control money-form" value="<?=$item['price']?>" id="price" name="data[price]" placeholder="Giá Tour">
                                </div>
                            </div>
                            <?php } ?>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control money-form" value="<?=$item['price_old']?>" id="price_old" name="data[price_old]" placeholder="Giá cũ">
                                </div>
                            </div>
                            <?php } ?>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="<?=$item['code']?>" id="code" name="data[code]" placeholder="Mã Tour">
                                </div>
                            </div>
                            <?php } ?>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="<?=$item['qty']?>" id="qty" name="data[qty]" placeholder="Số lượng khách ">
                                </div>
                            </div>
                            <?php } ?>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="font-weight-600">Link ảnh 3D</label>
                                    <input type="text" class="form-control" value="<?=$item['link']?>" id="link" name="data[link]" placeholder="Chèn link ảnh 3D">
                                </div>
                            </div>
                        </div>
                        <?php } ?>
                        ...
                        <?php 
                          $resTimeOfDays = $d->rawQuery("select name_vi as name,id from table_product_properties where type=? and find_in_set('hienthi',status) limit 30",array('time-of-days'));

                          $resTourLine = $d->rawQuery("select name_vi as name,id from table_product_properties where type=? and find_in_set('hienthi',status) limit 30",array('tour-line'));
  
                          if($_REQUEST['act']=='edit'){
                                $arr_tourLine = explode(',',$item['product_tour_line']);
                                $arr_timeOfDays = explode(',',$item['product_time_of_days']);
                          }  
                         ?>
                         <div class="row">
                            <?php if($csetting['time-of-days']==true) { ?>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="font-weight-600">Thời gian: </label>
                                    <select class="form-control basic-single" name="time-of-days[]">
                                        <option>Chọn thời gian</option>
                                        <?php if(!empty($resTimeOfDays)) { foreach($resTimeOfDays as $tod ) { ?>
                                        <option value="<?php echo $tod['id'] ?>" <?php echo (in_array($tod['id'],$arr_timeOfDays)) ? 'selected': ''; ?> ><?=$tod['name']?></option>
                                        <?php } } ?>
                                    </select>
                                </div>
                            </div>
                            <?php } ?>
                            <?php if($csetting['tour-line']==true) { ?>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="font-weight-600">Dòng tour: </label>
                                    <select class="form-control basic-single" name="tour-line[]">
                                        <option>Chọn dòng tour</option>
                                        <?php if(!empty($resTourLine)) { foreach($resTourLine as $tl ) { ?>
                                        <option value="<?php echo $tl['id'] ?>" <?php echo (in_array($tl['id'],$arr_tourLine)) ? 'selected': ''; ?> ><?=$tl['name']?></option>
                                        <?php } } ?>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="form-group lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                                    ...
                                    <textarea rows="4" cols="80" <?php if($csetting['mota-ck']==true) { ?>class="ck_editor" data-height="500"<?php }else{ ?>class="form-control"<?php } ?> name="data[desc_<?=$k?>]" id="desc_<?=$k?>"  placeholder="Nhập mô tả"><?=$item['desc_'.$k]?></textarea>
                                </div>
                                <?php } ?>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="form-group lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                                    ...
                                    <textarea rows="4" cols="80" <?php if($csetting['noidung-ck']==true) { ?>class="ck_editor" data-height="500"<?php }else{ ?>class="form-control"<?php } ?> name="data[content_<?=$k?>]" id="content_<?=$k?>"  placeholder="Nhập nội dung"><?=$item['content_'.$k]?></textarea>
                                </div>
                                <?php } ?>
                            </div>
                        </div>
                        <?php } ?>
                        <?php if($csetting['thongso']==true) { ?>
                        <div class="row">
                            <div class="col-md-12">
                                <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                <div class="form-group lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                                    <label class="font-weight-600">Thông số kỹ thuật [<?=$v?>]</label>
                                    <textarea rows="4" cols="80" <?php if($csetting['thongso-ck']==true) { ?>class="ck_editor" data-height="500"<?php }else{ ?>class="form-control"<?php } ?> name="data[specifications_<?=$k?>]" id="specifications_<?=$k?>"  placeholder="Nhập Thông số kỹ thuật"><?=$item['specifications_'.$k]?></textarea>
                                </div>
                                <?php } ?>
                            </div>
                        </div>
