<?php 
	...
	function get_items(){
		...
		if($config['paging-table']==true){
		    $items = $d->rawQuery("SELECT * from #_videos where type=? order by numb asc, id desc",$param);
		}else{
			$per_page = 10;
	        $startpoint = ($page * $per_page) - $per_page;
	        $limit = ' limit '.$startpoint.','.$per_page;
	        $sql = "SELECT * from #_videos where type=? order by numb asc, id desc ".$limit;
	        $items = $d->rawQuery($sql,$param);
	        $sqlq = "SELECT COUNT(*) as `num` from #_videos where type=? order by numb asc, id desc";
	...
	function delete_item(){
		...
		if(isset($_GET['id'])){
			$item  =  $d->rawQueryOne("select id,photo,thumb from #_videos where id=?",array($id));
			if($item){
