<?php 
	...
	function getListPage(){
		global $d,$items_list,$items_cat,$items_item,$items_sub,$item,$items_photo,$items_tags_type,$setting;
	...
	function get_items(){
	    ...
	    if($config['paging-table']==true){
		    $items = $d->rawQuery("SELECT * from #_products where type=? order by numb asc, id desc",$param);
		}else{
			$per_page = 10;
	...
	function save_item(){
    	...
    	$tourLines  = $_POST['tour-line'];
	    $arr_tourLine  = array();
	    if(!empty($tourLines)){
           foreach($tourLines as $v){
           	  if($v){
                 $arr_tourLine[] = $v; 
           	  }
           }
	    }
	    $arr_tourLine = array_filter($arr_tourLine);
	    $data['product_tour_line'] = implode(',',$arr_tourLine);


	$timeOfDays = $_POST['time-of-days'];
	    $arr_timeOfDays = array();
	    if(!empty($timeOfDays)){
           foreach($timeOfDays as $v){
           	  if($v){
                 $arr_timeOfDays[] = $v; 
           	  }
           }
	    }
	    $arr_timeOfDays = array_filter($arr_timeOfDays);
	    $data['product_time_of_days'] = implode(',',$arr_timeOfDays);
