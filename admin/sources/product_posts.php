<?php 
	...
	function save_item(){
    	...
    	$typeShippings     = $_POST['typeShipping'];
	    $arr_typeShipping  = array();
	    if(!empty($typeShippings)){
           foreach($typeShippings as $v){
           	  if($v){
                 $arr_typeShipping[] = $v; 
           	  }
           }
	    }
	    $arr_typeShipping = array_filter($arr_typeShipping);
	    $data['typeShipping'] = implode(',',$arr_typeShipping);


	    $holidayBeams = $_POST['holidayBeam'];
	    $arr_holidayBeam  = array();
	    if(!empty($holidayBeams)){
           foreach($holidayBeams as $v){
           	  if($v){
                 $arr_holidayBeam[] = $v; 
           	  }
           }
	    }
	    $arr_holidayBeam = array_filter($arr_holidayBeam);
	    $data['holidayBeam'] = implode(',',$arr_holidayBeam);
