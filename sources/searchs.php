<?php 
	...
	/*Kiểm tra danh mục*/
	if(!empty($alias) && $alias!=$type){
		$row_list = $d->rawQueryOne("select id from #_lists where type=? and alias_$lang=? and find_in_set ('hienthi',status)",array($type,$alias));
		if($row_list){
			$where = ' and id_list='.$row_list['id'];
		}else{
			$row_cat = $d->rawQueryOne("select id from #_cats where type=? and alias_$lang=? and find_in_set ('hienthi',status)",array($type,$alias));
			if($row_cat){
				$where = ' and id_cat='.$row_cat['id'];
			}else{
				$row_item = $d->rawQueryOne("select id from #_items where type=? and alias_$lang=? and find_in_set ('hienthi',status)",array($type,$alias));
				if($row_item){
	...
	$field_load = "name_$lang as name, alias_$lang as alias, id, photo, thumb, title_$lang as title, keywords_$lang as keywords, description_$lang as description, price, price_old";
	...
	if($func->isAjax()){
		...
		$html['resx'] = $_GET;
		$html['res'] = $func->getTemplateProductAll($items,'col--4 item',1,1);
		$html['page'] = $paging;
