<?php 
	if($func->isAjax()){
		...
		$row_detail = $d->rawQueryOne("select id,view,code,alias_$lang as alias,price,price_old,photo,thumb,name_$lang as name,desc_$lang as descs,content_$lang as content,id_list,status from #_products where id=? and find_in_set('hienthi',status)",array($id));
		...
		$row_detail['descs'] = $row_detail['descs'];
