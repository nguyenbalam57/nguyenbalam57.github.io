<?php 
	...
	$field_load = "name_$lang as name, alias_$lang as alias, desc_$lang as descs, id, photo, thumb, title_$lang as title, keywords_$lang as keywords, description_$lang as description,createdAt";
	...
	if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids) ){
		...
		$posts_other = $d->rawQuery("SELECT id,name_$lang as name, alias_$lang as alias, photo, thumb from #_posts where type=? and find_in_set ('hienthi',status) $where order by numb asc",array($type));
