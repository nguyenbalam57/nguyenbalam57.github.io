<?php 
	...
	$field_load = "name_$lang as name, alias_$lang as alias, desc_$lang as descs, id, photo, thumb, title_$lang as title, keywords_$lang as keywords, description_$lang as description";
	...
	if(!empty($idl)){
		...
		if($row_list){
			...
			$result_category = $d->rawQuery("select $field_breadcrumb from #_cats where type=? and id_list=? and tmp_table='products' and find_in_set ('hienthi',status)",array($type,$row_list['id']));
		}
	}
	if(!empty($idc)){
		$field_load .= ', id_list';
		...
		if($row_cat){
			...
			$result_category = $d->rawQuery("select $field_breadcrumb from #_items where type=? and id_cat=? and tmp_table='products' and find_in_set ('hienthi',status)",array($type,$row_cat['id']));

			if(!$result_category){
				$result_category = $d->rawQuery("select $field_breadcrumb from #_cats where type=? and id_list=? and tmp_table='products' and find_in_set ('hienthi',status)",array($type,$row_cat['id_list']));
			}
		}
	}
	if(!empty($idi)){
		...
		if($row_item){
			...
			$result_category = $d->rawQuery("select $field_breadcrumb from #_subs where type=? and id_item=? and tmp_table='products' and find_in_set ('hienthi',status)",array($type,$row_item['id']));
		}
	}
	if(!empty($ids)){
	...
	if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids) ){
		...
		$field_load .= ", price, price_old, id_list,id_cat,id_item,id_sub,tags,code,status, specifications_$lang as specifications, desc_$lang as descs, content_$lang as content,createdAt,updatedAt,type";
		...
		$posts = $d->rawQuery("select id,name_$lang as name, content_$lang as content from #_product_posts where type=? and id_product=? order by id desc",array($type,$id));
		...
		$data['breadcrumbs'][0] = array('alias'=>"mau-dong-phuc",'name'=>$title);
		...
		$product_other = $d->rawQuery("SELECT id,name_$lang as name, alias_$lang as alias, photo, thumb,price,price_old,desc_$lang as descs from #_products where type=? and find_in_set ('hienthi',status) $where order by numb asc",array($type));
		$product_color = $d->rawQuery("SELECT id,name_$lang as name, price, qty, photo, thumb from #_product_properties where type=? and id_product=? and find_in_set ('hienthi',status) order by numb asc",array('color',$row_detail['id']));
		$product_size = $d->rawQuery("SELECT id,name_$lang as name, price, qty, photo, thumb from #_product_properties where type=? and id_product=? and find_in_set ('hienthi',status) order by numb asc",array('size',$row_detail['id']));
		if($row_detail['tags']!='' && !empty($row_detail['tags'])){
			$tags_product = json_decode($row_detail['tags'],true);
              if($tags_product)
			$im_tag = implode(',',$tags_product);
          
			$product_tags = $d->rawQuery("SELECT id,name_$lang as name, alias_$lang as alias from #_tags where find_in_set ('hienthi',status) and id in ($im_tag) order by numb asc",array($type));
		}
